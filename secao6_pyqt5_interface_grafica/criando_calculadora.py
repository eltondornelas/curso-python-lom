...
class Calculadora(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('Calculadora do Elton')  # titulo
        self.setFixedSize(400, 400)  # fixa o tamanho, elimina para aumentar
        self.cw = QWidget()
        self.grid = QGridLayout(self.cw)

        self.display = QLineEdit()  # input
        self.grid.addWidget(self.display, 0, 0, 1, 5)
        self.display.setDisabled(True)  # desativa para não digitar nele
        self.display.setStyleSheet(
            '* {background: white; color: #000; font-size: 30px;}'
        )
        self.display.setSizePolicy(
            QSizePolicy.Preferred, QSizePolicy.Expanding)  # ocupa todo espaço

        self.add_btn(QPushButton('7'), 1, 0, 1, 1)  # display começa no 0
