...
class ChromeAuto:
    def __init__(self):
        ...
        )  # aqui é abrindo o chrome mesmo

    def clica_sign_in(self):
        try:
            btn_sign_in = self.chrome.find_element_by_link_text('Sign in')
            # caso não encontre o botão, vai acontecer uma exceção
            btn_sign_in.click()
    ...
    def clica_perfil(self):
        try:
            # quando faz uma copia do seletor ele retorna esse valor grande mesmo
...
if __name__ == '__main__':
    ...
    chrome.clica_perfil()
    chrome.verifica_usuario('eltondornelas')
