...
for root, dirs, files in os.walk(caminho_dos_pdfs):
    for file in files:
        # print(file)
        # ideal fazer a confirmação que o arquivo é um pdf
        caminho_completo_arquivo = os.path.join(root, file)

        # o correto seria abrir com o with open()... aqui, porém está dando algum problema com o PyPDF que não escreve nada
